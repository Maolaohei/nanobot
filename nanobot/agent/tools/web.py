...
class WebFetchTool(Tool):
    ...
    async def execute(self, url: str, extractMode: str = "markdown", maxChars: int | None = None, **kwargs: Any) -> str:
        ...
        try:
            ...
            # Cache lookup and conditional headers
            should_short_circuit, entry, cached_text, cond_headers = use_cache(self._cache, lambda h: None, url)
            if should_short_circuit and cached_text is not None:
                ...
                return json.dumps({
                    "url": url,
                    "finalUrl": url,
                    "status": entry.status if entry else 200,
                    "extractor": "cache",
                    "truncated": truncated,
                    "length": len(text),
                    "text": text,
            ...
            r = await http_request("GET", url, client=client, headers=headers)

            # Handle 304 Not Modified when revalidating
            ...
            if any(t in ctype for t in ("text/html", "application/json", "text/plain")):
                try:
                    self._cache.put(url, r.status_code, {k.lower(): v for k, v in r.headers.items()}, text)
                except Exception as ce:
                    logger.debug("cache put failed for {}: {}", url, ce)
